      ...
      <table class="min-w-full bg-white border-collapse">
        ...
        <tbody>
          ...
          >
            ...
            <!-- renderiza 'tipo' com segurança -->
            <td class="py-2 px-4 capitalize">
              {{ sessao.tipo ? sessao.tipo.toString().toLowerCase() : '' }}
            </td>
          </tr>
        </tbody>
      </table>
...
async function fetchSessoes() {
  ...
  try {
    ...
    // se sua API retornar { data: [...] }, descomente a linha abaixo:
    // sessoes.value = Array.isArray(response.data.data) ? response.data.data : []
    sessoes.value = Array.isArray(response.data) ? response.data : []
  } catch (err) {
...
onMounted(fetchSessoes)
</script>

<style scoped>
/* Estilos via Tailwind, nada extra */
</style>
