...
const props = defineProps({
  /** Visibilidade via v-model:visible */
  visible:    { type: Boolean, required: true },
  /** ID do vereador para buscar detalhes */
...
async function fetchDetails() {
  try {
    const [vRes, lRes, pRes, iRes] = await Promise.all([
      api.get(`/vereadores/${props.vereadorId}`),
      api.get(`/vereadores/${props.vereadorId}/legislaturas`),
      api.get(`/vereadores/${props.vereadorId}/projetos`),
      api.get(`/vereadores/${props.vereadorId}/indicacoes`)
    ])
