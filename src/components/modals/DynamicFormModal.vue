...
<!-- src/components/modals/DynamicFormModal.vue -->
<template>
...
const props = defineProps({
  /** Controla a visibilidade via v-model:visible */
...
function initForm() {
  props.fields.forEach(f => {
    // inicializa o valor do campo vazio
    form[f.field] = ''

    // se for select, carrega opções via API
    if (f.type === 'select' && f.optionEndpoint) {
      options[f.field] = []
      api
        .get(f.optionEndpoint)
        .then(({ data }) => {
          // suporta array puro ou { data: [...] }
...
// reinicia o form sempre que o modal for aberto
watch(
  () => props.visible,
  visible => {
    if (visible) initForm()
