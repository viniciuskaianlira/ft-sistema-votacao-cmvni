<template>
  <div class="p-6 bg-white min-h-full">
    ...
    <div class="mb-4">
      <label for="legislaturaSelect" class="block text-sm font-medium text-gray-700 mb-1">Selecionar Legislatura:</label>
      <select 
    ...
    <div class="mb-8 p-6 border rounded-lg shadow-sm bg-gray-50">
      <h2 class="text-xl font-semibold mb-4">Composição da {{ selectedLegislatura?.numero }}ª Legislatura</h2>
      <div v-if="loading">Carregando composição...</div>
      <div v-else-if="!mesaDiretora || mesaDiretora.length === 0">Nenhuma composição definida para a legislatura selecionada.</div>
    ...
    <div class="mb-8 p-6 border rounded-lg shadow-sm bg-gray-50">
        <h2 class="text-xl font-semibold mb-4">Gerenciar Composição da {{ selectedLegislatura?.numero }}ª Legislatura</h2>
        <p class="text-sm text-gray-600 mb-4">Selecione os vereadores para cada cargo da Mesa Diretora.</p>
        
        <Form @submit="handleCompositionSubmit" :validation-schema="compositionSchema" v-slot="{ errors, isSubmitting, resetForm: veeResetForm }">
